...
Project context
---------------
After the data-processing refactor, the training pipeline consumes a unified
training artifact generated by the data_processing block.
...
def materialize_experiment_assets(cfg: DictConfig) -> None:
    ...
    if "assets" not in cfg and "experiment" not in cfg:
        return
    ...
    logger.info(
        "Materializing experiment assets | manifest=%s | base_artifact=%s",
        manifest_path,
        base_artifact_path,
    )
...
def resolve_experiment_overlay(cfg: DictConfig) -> DictConfig:
    ...
    if isinstance(experiment_ref, DictConfig):
        overlay_container = OmegaConf.to_container(
            _strip_hydra_defaults(experiment_ref),
            resolve=False,
        )
    elif isinstance(experiment_ref, (str, Path)):
        experiment_name = str(experiment_ref)
        project_root = Path(__file__).resolve().parents[2]
        candidate_path = Path(experiment_name)
        if not candidate_path.suffix:
            candidate_path = project_root / "configs" / "experiments" / f"{experiment_name}.yaml"
        ...
        overlay_container = OmegaConf.to_container(overlay, resolve=False)
    else:
        return cfg

    if not isinstance(overlay_container, dict):
        return cfg
...
def resolve_training_artifact_path(cfg: DictConfig) -> Path:
    ...
    if not artifact_path:
        raise ConfigurationContractError(
            "Could not resolve training artifact path. Please define 'artifact.path' in your configuration."
        )
